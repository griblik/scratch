...
def clusterWithMostSamples(model):
  ...
  for i in range(len(model.cluster_centers_)):
    if minSamples <= (model.labels_==i).sum():
      minCluster = i
      minSamples = (model.labels_==i).sum()
  print("\n  Cluster With Most Samples: ", minCluster)
#  return (model.labels_==minCluster)
...
def snoopPerson(person):
    if person:
        print("\n\nExamining person: ", person)
        
        user = df.loc[df['In'] == person]
        user = user.loc[user['CallDate'].dt.dayofweek <= 4]
        user = user[user['CallTime'] <= '17:00:00']
        
        fig = plt.figure()
        ax = fig.add_subplot(111)
        ax.scatter(user.TowerLon, user.TowerLat, c='g', marker='.', alpha=0.2)
        showandtell(title='User %(num)d Cell towers' % {'num':person})
        ...
        ax.set_title('Kmeans for %(num)d' % {'num':person})
        plt.show()
        print(model.cluster_centers_)
        
        
    else:
        print('No person to snoop')

for person in inlist:
    snoopPerson(person)
...
user1 = user1.loc[user1['CallDate'].dt.dayofweek <= 4]
...
fig = plt.figure()
ax = fig.add_subplot(111)
ax.scatter(user1.TowerLon, user1.TowerLat, c='g', marker='.', alpha=0.2)
showandtell(title='Cell towers connected to')
